...
@section('title')
    @if($category)
        {{$category->title}} - описание, фото, черенки и саженцы, купить в Минске, Беларусь. Отправим заказ почтой и ТК.{{$page ? ' Страница - ' . $page : ''}}
    @else
        Черенки и саженцы винограда купить в Минске, Беларусь. Отправим заказ почтой и ТК. Полный каталог сортов.{{$page ? ' Страница - ' . $page : ''}}
    @endif
@endsection
@section('key', $category ? $category->meta_key : 'виноград, черенки и саженцы, купить в Минске, Беларусь, отправка почтой, отправка ТК')
@section('desc')
    @if($category)
        {{$category->meta_desc}}{{$page ? ' Страница - ' . $page : ''}}
    @else
        Черенки и саженцы винограда купить в Минске, Беларусь. Отправляем заказы по Беларуси почтой, в РФ отправляем как обычной почтой, так и ТК. Полный каталог сортов.{{$page ? ' Страница - ' . $page : ''}}
    @endif
@endsection
